#!/usr/bin/env python3
"""Import operadores de polo SEI → Univesp Staff Directory (+ sync Access Profile).

Fonte: funcionario + funcionariocargo (@polo.univesp.br).

Uso:
  python ops/import/staff-from-trino.py --validate-env
  python ops/import/staff-from-trino.py --dry-run --limit 50
  python ops/import/staff-from-trino.py --apply --site crm.localhost --full --sync-profiles
"""
...
JOIN "postgresql-sei".public.funcionario f
  ON f.pessoa = p.codigo
JOIN "postgresql-sei".public.funcionariocargo fc
  ON fc.funcionario = f.codigo
JOIN "postgresql-sei".public.unidadeensino ue
  ON ue.codigo = fc.unidadeensino
...
def main() -> int:
	load_trino_env()
	parser = argparse.ArgumentParser(description="Import operadores polo via Trino (SEI → Staff Directory)")
	...
	parser.add_argument("--sync-profiles", action="store_true", help="Atualiza Univesp Access Profile (op) apos import")
	...
	query = build_query(polo_ids=args.polo_ids, limit=limit, active_only=not args.include_inactive)
	raw_rows = fetch_rows(query)
	rows = sanitize_rows(raw_rows)
	emails = len({row["email"] for row in rows})
	print(json.dumps({"staff_rows": len(rows), "unique_emails": emails}, ensure_ascii=False, indent=2))
	...
	if args.apply:
		...
		result = apply_via_bench(
			args.site,
			rows,
			method="univesp_atendimento.import_staff.upsert_rows",
			batch_size=max(args.batch_size, 1),
		)
		print(json.dumps(result, ensure_ascii=False, indent=2))
		if args.sync_profiles:
			sync = run_bench_execute(
				args.site,
				"univesp_atendimento.import_staff.sync_access_profiles_from_staff_directory",
			)
			print(json.dumps({"sync_profiles": sync}, ensure_ascii=False, indent=2))
		return 0
