...
class HomologReadinessToolingTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.workflow = (ROOT / ".github" / "workflows" / "univesp-cloudrun-homolog.yml").read_text(encoding="utf-8")

    def test_preflight_is_read_only_and_checks_dependencies(self):
        for required in ("artifacts repositories describe", "sql instances describe", "storage buckets describe",
                         "vpc-access connectors describe", "service-accounts describe",
    ...
    def test_manifest_captures_all_runtime_images_without_secret_values(self):
        for name in ("WEB_SERVICE", "WORKER_SERVICE", "SCHEDULER_SERVICE", "GATEWAY_SERVICE", "BOOTSTRAP_JOB"):
    ...
    def test_smoke_is_read_only_and_covers_front_door(self):
        for path in ("/api/me", "/api/app/v1/tickets", "/api/method/ping", "/api/resource/User",
                     "/desk", "/files/private.txt", "/socket.io/", "/api/sso/azure/start",
                     "/api/sso/saml/start"):
    ...
    def test_workflow_enforces_preflight_smoke_and_evidence_upload(self):
        self.assertLess(self.workflow.index("Run read-only GCP preflight"), self.workflow.index("Build and push Frappe image"))
        self.assertGreater(self.workflow.index("Collect post-deploy homolog evidence"), self.workflow.index("Deploy Frappe services"))
