...
class CloudRunFrontDoorTest(unittest.TestCase):
	...
	def test_cloudrun_image_contains_academic_apps(self):
		self.assertIn("rm -rf apps/helpdesk", self.containerfile)
		...
		self.assertIn('fetch --depth 1 origin "${HELPDESK_REF}"', self.containerfile)
		self.assertNotIn('fetch --depth 1 "${HELPDESK_REPOSITORY}" "${HELPDESK_REF}"', self.containerfile)
		self.assertIn('rev-parse HEAD)" = "${HELPDESK_REF}"', self.containerfile)
		self.assertIn("pip install --no-cache-dir --editable apps/helpdesk", self.containerfile)
	...
	def test_gateway_is_built_and_deployed_with_managed_secrets(self):
		...
		self.assertIn("Redis access provisioner fingerprint:", redis_access)
		self.assertIn("sha256sum", redis_access)
		...
		self.assertIn("helpdesk_ref", self.workflow)
