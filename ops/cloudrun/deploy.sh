...
if [[ -z "${PROJECT_ID}" || -z "${IMAGE_URI}" || -z "${CLOUDSQL_INSTANCE}" || -z "${SITES_BUCKET}" || -z "${VPC_CONNECTOR}" || -z "${RUNTIME_SERVICE_ACCOUNT}" || -z "${SSO_GATEWAY_ORIGIN}" || -z "${FRAPPE_SERVICE_USER_EMAIL}" || -z "${INITIAL_ADMIN_EMAIL}" ]]; then
	printf 'GCP_PROJECT_ID, IMAGE_URI, CLOUDSQL_INSTANCE, SITES_BUCKET, VPC_CONNECTOR, CLOUDRUN_RUNTIME_SERVICE_ACCOUNT, SSO_GATEWAY_ORIGIN, FRAPPE_SERVICE_USER_EMAIL and INITIAL_ADMIN_EMAIL are required.\n' >&2
	exit 1
fi
