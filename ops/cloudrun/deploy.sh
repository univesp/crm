...
PUBLIC_EMAIL_REPLY_SECRET_NAME=${PUBLIC_EMAIL_REPLY_SECRET_NAME:-crm-homolog-public-email-reply-secret}
SMTP_PASSWORD_SECRET_NAME=${SMTP_PASSWORD_SECRET_NAME:-crm-homolog-smtp-password}
...
ANTIMALWARE_ENDPOINT=${ANTIMALWARE_ENDPOINT:-}
MEDIA_PROCESSOR_ENDPOINT=${MEDIA_PROCESSOR_ENDPOINT:-}
PUBLIC_DOCUMENT_RETENTION_DAYS=${PUBLIC_DOCUMENT_RETENTION_DAYS:-180}
PUBLIC_UPLOAD_TTL_HOURS=${PUBLIC_UPLOAD_TTL_HOURS:-24}
PUBLIC_REPLY_DOMAIN=${PUBLIC_REPLY_DOMAIN:-}
SMTP_HOST=${SMTP_HOST:-}
SMTP_PORT=${SMTP_PORT:-587}
SMTP_USERNAME=${SMTP_USERNAME:-}
SMTP_FROM_EMAIL=${SMTP_FROM_EMAIL:-}
SMTP_FROM_NAME=${SMTP_FROM_NAME:-Atendimento UNIVESP}
SMTP_USE_TLS=${SMTP_USE_TLS:-true}
SMTP_USE_SSL=${SMTP_USE_SSL:-false}
SMTP_NO_AUTHENTICATION=${SMTP_NO_AUTHENTICATION:-false}

if [[ -z "${PROJECT_ID}" || -z "${IMAGE_URI}" || -z "${CLOUDSQL_INSTANCE}" || -z "${SITES_BUCKET}" || -z "${VPC_CONNECTOR}" || -z "${RUNTIME_SERVICE_ACCOUNT}" || -z "${SSO_GATEWAY_ORIGIN}" || -z "${ANTIMALWARE_ENDPOINT}" || -z "${MEDIA_PROCESSOR_ENDPOINT}" || -z "${FRAPPE_SERVICE_USER_EMAIL}" || -z "${INITIAL_ADMIN_EMAIL}" ]]; then
	printf 'GCP_PROJECT_ID, IMAGE_URI, CLOUDSQL_INSTANCE, SITES_BUCKET, VPC_CONNECTOR, CLOUDRUN_RUNTIME_SERVICE_ACCOUNT, SSO_GATEWAY_ORIGIN, ANTIMALWARE_ENDPOINT, MEDIA_PROCESSOR_ENDPOINT, FRAPPE_SERVICE_USER_EMAIL and INITIAL_ADMIN_EMAIL are required.\n' >&2
	exit 1
fi

for endpoint in "${SSO_GATEWAY_ORIGIN}" "${ANTIMALWARE_ENDPOINT}" "${MEDIA_PROCESSOR_ENDPOINT}"; do
	case "${endpoint}" in
	https://*) ;;
	*)
		printf 'Gateway and FAQ service endpoints must use https://.\n' >&2
		exit 1
		;;
	esac
done
...
common_env="${common_env},GCS_MOUNTED_STORAGE=true,GCS_BUCKET=${SITES_BUCKET},FAQ_CLOUD_RUN_IAM_AUTH=true"
common_env="${common_env},ANTIMALWARE_ENDPOINT=${ANTIMALWARE_ENDPOINT},MEDIA_PROCESSOR_ENDPOINT=${MEDIA_PROCESSOR_ENDPOINT},PUBLIC_DOCUMENT_RETENTION_DAYS=${PUBLIC_DOCUMENT_RETENTION_DAYS},PUBLIC_UPLOAD_TTL_HOURS=${PUBLIC_UPLOAD_TTL_HOURS}"
common_env="${common_env},DEPLOYMENT_ENV=${DEPLOYMENT_ENV:-},FAQ_V3_HOMOLOG_PILOT_ENABLED=${FAQ_V3_HOMOLOG_PILOT_ENABLED:-false}"
if [[ "${DEPLOYMENT_ENV:-}" == "homolog" ]]; then
	common_env="${common_env},SMTP_BOOTSTRAP_SKIP_LIVE_PROBE=true"
fi
web_env=${common_env}
...
faq_service_secrets="ANTIMALWARE_TOKEN=${ANTIMALWARE_TOKEN_SECRET_NAME}:latest,MEDIA_PROCESSOR_TOKEN=${MEDIA_PROCESSOR_TOKEN_SECRET_NAME}:latest"
email_secrets="UNIVESP_INGRESS_SHARED_SECRET=${INGRESS_SHARED_SECRET_NAME}:latest,PUBLIC_EMAIL_REPLY_SECRET=${PUBLIC_EMAIL_REPLY_SECRET_NAME}:latest,SMTP_PASSWORD=${SMTP_PASSWORD_SECRET_NAME}:latest"
if [[ "${FAQ_V3_HOMOLOG_PILOT_ENABLED:-false}" == "true" ]]; then
	for name in PUBLIC_REPLY_DOMAIN SMTP_HOST SMTP_PORT SMTP_FROM_EMAIL; do
		if [[ -z "${!name:-}" ]]; then
			printf 'Missing required FAQ public email variable: %s\n' "${name}" >&2
			exit 1
		fi
	done
	if [[ "${SMTP_NO_AUTHENTICATION}" != "true" && -z "${SMTP_USERNAME:-}" ]]; then
		printf 'Missing required FAQ public email variable: SMTP_USERNAME\n' >&2
		exit 1
	fi
	common_env="${common_env},PUBLIC_REPLY_DOMAIN=${PUBLIC_REPLY_DOMAIN},SMTP_HOST=${SMTP_HOST},SMTP_PORT=${SMTP_PORT},SMTP_USERNAME=${SMTP_USERNAME:-},SMTP_FROM_EMAIL=${SMTP_FROM_EMAIL},SMTP_FROM_NAME=${SMTP_FROM_NAME},SMTP_USE_TLS=${SMTP_USE_TLS},SMTP_USE_SSL=${SMTP_USE_SSL},SMTP_NO_AUTHENTICATION=${SMTP_NO_AUTHENTICATION}"
fi
...
gcloud run jobs deploy "${BOOTSTRAP_JOB}" \
	...
	--cpu 2 \
	--memory 4Gi \
	--set-env-vars "${common_env},DB_NAME=${DB_NAME},DB_USER=${DB_USER},DB_ROOT_USERNAME=${DB_ROOT_USERNAME},FRAPPE_SERVICE_USER_EMAIL=${FRAPPE_SERVICE_USER_EMAIL},INITIAL_ADMIN_EMAIL=${INITIAL_ADMIN_EMAIL}" \
...
gcloud run deploy "${WEB_SERVICE}" \
	...
	--set-secrets "${redis_secrets},${faq_service_secrets},UNIVESP_EDGE_SHARED_SECRET=${EDGE_SHARED_SECRET_NAME}:latest" \
