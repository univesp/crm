...
if [[ "${FAQ_V3_HOMOLOG_PILOT_ENABLED:-false}" == "true" ]]; then
	for name in PUBLIC_REPLY_DOMAIN SMTP_HOST SMTP_PORT SMTP_USERNAME SMTP_FROM_EMAIL; do
		if [[ -z "${!name:-}" ]]; then
			printf 'Missing required FAQ public email variable: %s\n' "${name}" >&2
			exit 1
		fi
	done
	common_env="${common_env},PUBLIC_REPLY_DOMAIN=${PUBLIC_REPLY_DOMAIN},SMTP_HOST=${SMTP_HOST},SMTP_PORT=${SMTP_PORT},SMTP_USERNAME=${SMTP_USERNAME},SMTP_FROM_EMAIL=${SMTP_FROM_EMAIL},SMTP_FROM_NAME=${SMTP_FROM_NAME},SMTP_USE_TLS=${SMTP_USE_TLS},SMTP_USE_SSL=${SMTP_USE_SSL}"
fi
