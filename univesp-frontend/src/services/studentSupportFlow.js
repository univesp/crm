...
export function buildFaqAttendanceContext({ node, lineage, sessionId, currentDate = new Date() }) {
  ...
  return {
    ...
    action: node.acao,
    responseMode: node.response_mode || 'informational',
    academicIntent: node.academic_intent || 'none',
    dataContractKey: node.data_contract_key || '',
    confidencePolicy: node.confidence_policy || 'answer_when_deterministic',
    queueDestination: queueDestinationForRouting,
    criticality: node.criticidade_padrao,
    sla: node.sla_padrao,
    ownership: {
...
export function buildProtocolDraft({ context, sourceRecordId, currentDate = new Date() }) {
  ...
  return {
    ...
    form: {
      ...
      responseMode: context.responseMode || 'informational',
      academicIntent: context.academicIntent || 'none',
      dataContractKey: context.dataContractKey || '',
      confidencePolicy: context.confidencePolicy || 'answer_when_deterministic',
      queueDestination: context.queueDestination,
      ownerType: context.ownership?.ownerType || '',
