...
from univesp_atendimento.univesp_atendimento.doctype.univesp_staff_directory.univesp_staff_directory import build_staff_key
...
def upsert_rows(rows=None):
	...
	for raw in payload:
		...
		if not email or "@" not in email or not polo_id:
			skipped += 1
			continue
		values = {
...
def sync_access_profiles_from_staff_directory():
	...
	rows = frappe.get_all("Univesp Staff Directory", filters={"cargo_ativo": 1}, fields=["email", "nome", "polo_id"], limit_page_length=0)
